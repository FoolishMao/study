...
human_files_detect = [file for file in human_files_short if face_detector(file) is True]
dog_files_detect = [file for file in dog_files_short if face_detector(file) is True]
human_files_acc_rate = len(human_files_detect) / len(human_files_short)
dog_files_acc_rate = len(dog_files_detect) / len(dog_files_short)
##分别打印正确率
print('the accuracy rate detected in human_files_short is ','\n',human_files_acc_rate * 100,'%')
print('the accuracy rate detected in dog_files_short is ','\n',dog_files_acc_rate * 100,'%')
...
# 
# __回答:__减小卷积窗的维度，增加卷积层的维度和数量以提取更多更细化的特征。
# 
# 
...
human_files_dog_detect = [file for file in human_files_short if dog_detector(file) == 1]
dog_files_dog_detect = [file for file in dog_files_short if dog_detector(file) == 1]
human_files_dogacc_rate = len(human_files_dog_detect) / len(human_files_short)
dog_files_dogacc_rate = len(dog_files_dog_detect) / len(dog_files_short)

##分别打印正确率
print('the dog rate detected in human_files_short is ','\n',human_files_dogacc_rate * 100,'%')
print('the dog rate detected in dog_files_short is ','\n',dog_files_dogacc_rate * 100,'%')
...
# 
# __回答:__ 卷积窗的大小为2*2，可以较细的提取特征。一共使用三个卷积层，第一层用于提取简单的例如线条等简单规律，第二层用以提取较为复杂的规律，第三层用以提取特征。使用池化层和GAP层有效的减少参数值，最后使用全连接层输出分类结果。
...
# 
# 
# __回答:__ 1.使用全局平均池化层，将输入的张量转化为向量，相较于使用展平层，可以有效的减少模型的参数。
# 2.添加神经网络的隐藏层以发现更加典型的规律。
# 3.添加Dropout使得在较高的epoch下不出现过拟合。
# 4.最后添加输出层，以预测狗狗种类。
# 
# 
...
def photo_detect(img_path):
    if dog_detector(img_path) == 1:
        plt.imshow(mpimg.imread(img_path))
        print(VGG19_predict_breed(img_path))
    elif face_detector(img_path) == 1:
        print('hello,human!','\n')
        plt.imshow(mpimg.imread(img_path))
        print('\n','You look like a ...','\n',VGG19_predict_breed(img_path))
    else:
        plt.imshow(mpimg.imread(img_path))
        print('\n','Not a human or dog')
...
# 同时请回答如下问题：
# 
# 1. 输出结果比你预想的要好吗 :) ？或者更糟 :( ？     要好，只有一个错误
# 2. 提出至少三点改进你的模型的想法。
# 1）模型出现了过拟合的情况，优化函数可以设置惩罚项
# 2）适当调整学习率
# 3）适当调整batch数
# 4）采用图像强化的方法
...
## 自由地使用所需的代码单元数吧
photo_files = np.array(glob("photo_detect/*"))
...
photo_detect(photo_files[0])


# In[37]:

photo_detect(photo_files[1])


# In[38]:

photo_detect(photo_files[2])


# In[40]:

photo_detect(photo_files[3])


# In[41]:

photo_detect(photo_files[4])


# In[42]:

photo_detect(photo_files[5])
